...
function onRangeVolChange(value, local) {
    // UI shows Volume from 0 mute to 23 max no log10 conversion
    document.getElementById('vol1_span').innerHTML = value;
    document.getElementById('vol_span').innerHTML = value;
...
function wifi(valid) {
	...
	xhr.onreadystatechange = function () {
		if (xhr.readyState == 4 && xhr.status == 200) {
			...
			document.getElementById('BTname').value = arr["BTname"];
			document.getElementById('BTpass').value = arr["BTpass"];
	...
	var debug = "valid=" + valid
		...
		+ "&BTname=" + encodeURIComponent(document.getElementById('BTname').value)
		+ "&BTpass=" + encodeURIComponent(document.getElementById('BTpass').value) + "&";
	console.log(debug);
	xhr.send(debug);
}
function btForget() {
	// Send CLI command to backend
	websocket.send("forget");
	alert('All bonded Bluetooth devices will be deleted.');
}
function validateBT() {
	var btname = document.getElementById("BTname").value;
	var btpass = document.getElementById("BTpass").value;
	var xhr = new XMLHttpRequest();
	xhr.onreadystatechange = function () {
		if (xhr.readyState == 4 && xhr.status == 200) {
			alert("Bluetooth settings updated!");
		}
	};
	xhr.open("POST", "hardware", true);
	xhr.setRequestHeader(content, ctype);
	xhr.send("valid=3&BTname=" + encodeURIComponent(btname) + "&BTpass=" + encodeURIComponent(btpass));
}
function hardware(valid) {
	...
	xhr.onreadystatechange = function () {
		if (xhr.readyState == 4 && xhr.status == 200) {
			...
			if (arr["BTenabled"] != "1") {
				document.getElementById("bt_section").style.display = "none";
			} else {
				document.getElementById("bt_section").style.display = "block";
			}
		}
	}
...
function autostart() {
	...
	autoplay(); // to apply the state to the backend
}
...
function radiotoggle() {
	...
	alert('Toggle between WebRadio and Bluetooth Mode.');
}
...
function uploadFirmware() {
	...
	var updatefb = document.getElementById('updatefb');
	if (!fileInput.files.length) {
		updatefb.innerText = 'Select a .bin file!';
		return;
	}
	var file = fileInput.files[0];
	if (file.size > 2015232) { // 1964K + some margin // limit for 4MB flash adjust this if your partitions are bigger
		updatefb.innerText = 'File too large! Max 1964K.';
		return;
	}
	// Check if device has a display. If yes: show PIN modal and request PIN on device.
	var hx = new XMLHttpRequest();
	hx.open('GET', '/has_display', true);
	hx.onload = function () {
		if (hx.status === 200 && hx.responseText.trim() === '1') {
			// show PIN modal and request device to display PIN
			showPinModal();
			// request the device to generate and show PIN
			generatePin();
		} else {
			// no display -> upload immediately without PIN header
			doUpload('');
		}
	};
	hx.onerror = function () {
		// On error assume no display and upload
		doUpload('');
	};
	hx.send();
}

// Request device to generate & display a one-time PIN (do not return PIN over network)
function generatePin() {
	var updatefb = document.getElementById('updatefb');
	var xhr = new XMLHttpRequest();
	xhr.open('GET', '/generate_pin', true);
	xhr.onload = function () {
		if (xhr.status === 200) {
			updatefb.innerText = 'PIN displayed on device. Enter it here and press OK.';
			var mf = document.getElementById('modalFb');
			if (mf) mf.innerText = 'PIN displayed on device.';
		} else {
			updatefb.innerText = 'Could not request PIN.';
			var mf = document.getElementById('modalFb');
			if (mf) mf.innerText = 'Could not request PIN.';
		}
	};
	xhr.onerror = function () { updatefb.innerText = 'Request failed.'; document.getElementById('modalFb').innerText = 'Request failed.'; };
	xhr.send();
	updatefb.innerText = 'Requesting PIN...';
}
...
function showPinModal() {
	...
	var pin = document.getElementById('modalPin');
	if (pin) { pin.value = ''; pin.focus(); }
	var mf = document.getElementById('modalFb'); if (mf) mf.innerText = '';
}
...
function hidePinModal() {
	...
	document.getElementById('updatefb').innerText = '';
}

function uploadFirmwareWithPin() {
	var pin = document.getElementById('modalPin').value || '';
	if (pin && pin.length < 4) pin = pin.padStart(4, '0');
	// start upload with provided PIN (modal will be closed by doUpload)
	hidePinModal();
	doUpload(pin);
}

function doUpload(pin) {
	var fileInput = document.getElementById('fwfile');
	var updatefb = document.getElementById('updatefb');
	if (!fileInput.files.length) {
		updatefb.innerText = 'Select a .bin file!';
	...
	if (pin && pin.length > 0) {
		try { xhr.setRequestHeader('X-OTA-PIN', btoa(pin)); } catch (e) { updatefb.innerText = 'PIN encoding error'; return; }
	}
	xhr.onload = function () {
		updatefb.innerText = xhr.responseText;
		if (xhr.responseText.includes('OTA update succeeded.')) {
			setTimeout(function () { location.reload(); }, 15000);
		}
	};
	xhr.onerror = function () { updatefb.innerText = 'Upload failed.'; };
	xhr.send(file);
	updatefb.innerText = 'Uploading...';
}
