...
#define MONO
#elif defined(CONFIG_DISPLAY_TYPE_ALL)
#define MONO
#else 
#ifdef MONO
#undef MONO 
...
static void screenBottomU8g2()
{
	...
	u8g2_DrawHLine(&u8g2, 0, yy - 2, ((uint16_t)(x * volume) / 23));
	// TIME
...
// Draw OTA progress (called from drawScreen when stateScreen == sprogress)
void drawProgressU8g2(uint8_t percent)
{
	char s[16];
	snprintf(s, sizeof(s), "OTA %d%%", percent);
	setfont8(middle);
	uint16_t w = u8g2_GetUTF8Width(&u8g2, s);

	// Redraw whole page to avoid overlapping digits; draw background for title
	u8g2_ClearBuffer(&u8g2);
	u8g2_FirstPage(&u8g2);
	do
	{
		// draw title background box to clear previous text
		uint16_t th = getFontLineSpacing() + 4;
		u8g2_SetDrawColor(&u8g2, 1);
		u8g2_DrawBox(&u8g2, 0, yy / 3 - th + 2, x, th);
		u8g2_SetDrawColor(&u8g2, 0);
		u8g2_DrawUTF8(&u8g2, (x / 2) - (w / 2), yy / 3, s);
		...
		if (percent > 0)
		{
			...
			if (fillw > 2)
				u8g2_DrawBox(&u8g2, bx + 1, by + 1, fillw - 2, bh - 2);
		}
		vTaskDelay(pdMS_TO_TICKS(10));
	} while (u8g2_NextPage(&u8g2));
}
