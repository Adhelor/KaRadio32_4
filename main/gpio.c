...
spi_device_handle_t vsspi;  // the evice handle of the vs1053 spi
spi_device_handle_t hvsspi; // the device handle of the vs1053 spi high speed
#ifndef CONFIG_USE_VS1053
xSemaphoreHandle vsSPI = NULL;
xSemaphoreHandle hsSPI = NULL;
#else
extern xSemaphoreHandle vsSPI;
extern xSemaphoreHandle hsSPI;
#endif
...
void option_get_lcd_blv(int *blv)
{
	...
	*blv = 100;
	...
	else
	{
		...
		ESP_LOGV(TAG, "lcd_blv set successfully, value: %d", lout);
		*blv = lout;
	}
	close_partition(hardware_handle, hardware);
}
...
void gpio_set_ledgpio(gpio_num_t enca)
{
	...
	// err = nvs_set_u8(hardware_handle, "K_SPI", 2);
...
void gpio_get_adc(adc1_channel_t *channel, adc1_channel_t *chanbatt)
{
	...
	*channel = gpioToChannel(*channel);
	// battery gpio
	*chanbatt = PIN_BAT;
	*chanbatt = gpioToChannel(*channel);
	...
	if (err != ESP_OK)
		ESP_LOGW(TAG, "g_get_adc kbd err 0x%x", err);
	else
		*channel = gpioToChannel(*channel);
	...
	if (err == ESP_ERR_NVS_NOT_FOUND)
	{
		nvs_set_u8(hardware_handle, "P_ADC_BAT", PIN_BAT);
	}
	if (err != ESP_OK)
		ESP_LOGW(TAG, "g_get_adc bat err 0x%x", err);
	else
...
bool gpio_get_ir_key(nvs_handle handle, const char *key, uint32_t *out_value1, uint32_t *out_value2)
{
	...
	*out_value2 = 0;
	...
	if (required_size > 1)
	{
		...
		sscanf(string, "%lx %lx", out_value1, out_value2);
#else
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
	ESP_LOGV(TAG, "Key: %s, value1: %lx, value2: %lx, ret: %d", key, *out_value1, *out_value2, ret);
#else
...
#else
	ret = spi_bus_initialize(spi_no, &buscfg, SPI_DMA_CH_AUTO); // dma
#endif
	assert(ret == ESP_OK);
}
...
void gpio_get_bt_bonded_list(bt_bonded_list_t *list)
{
	nvs_handle handle;
	esp_err_t err;
	memset(list, 0, sizeof(bt_bonded_list_t));
	if (open_partition(hardware, BT_PAIRING_NAMESPACE, NVS_READONLY, &handle) != ESP_OK)
		return;
	for (int i = 0; i < MAX_BT_BONDS; ++i)
	{
		char mac_key[40], name_key[40];
		snprintf(mac_key, sizeof(mac_key), "BT_MAC_%d", i);
		snprintf(name_key, sizeof(name_key), "BT_NAME_%d", i);
		size_t mac_len = 6;
		size_t name_len = BT_BOND_NAME_LEN + 1;
		uint8_t mac[6] = {0};
		char name[BT_BOND_NAME_LEN + 1] = {0};
		err = nvs_get_blob(handle, mac_key, mac, &mac_len);
		err |= nvs_get_str(handle, name_key, name, &name_len);
		if (err == ESP_OK && mac_len == 6 && mac[0] != 0)
		{
			memcpy(list->devices[list->count].mac, mac, 6);
			strncpy(list->devices[list->count].name, name, BT_BOND_NAME_LEN + 1);
...
void gpio_set_bt_bonded_list(const bt_bonded_list_t *list)
{
	nvs_handle handle;
	if (list == NULL)
	{
		if (nvs_open(hardware, NVS_READWRITE, &handle) == ESP_OK)
		{
			nvs_erase_key(handle, "bt_pairing");
			nvs_commit(handle);
			nvs_close(handle);
		}
		return;
	}
	if (open_partition(hardware, BT_PAIRING_NAMESPACE, NVS_READWRITE, &handle) != ESP_OK)
	{
	...
	for (int i = 0; i < MAX_BT_BONDS; ++i)
	{
		...
		if (i < list->count)
		{
			nvs_set_blob(handle, mac_key, list->devices[i].mac, 6);
			nvs_set_str(handle, name_key, list->devices[i].name);
			ESP_LOGI(TAG, "Store: %s, %s", mac_key, name_key);
		}
		else
		{
			uint8_t zero_mac[6] = {0};
			nvs_set_blob(handle, mac_key, zero_mac, 6);
			nvs_set_str(handle, name_key, "");
		}
	}
	close_partition(handle, hardware);
}
