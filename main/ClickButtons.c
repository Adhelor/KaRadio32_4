...
IRAM_ATTR void serviceBtn(Button_t *enc)
{
	...
	for (uint8_t i = 0; i < 3; i++)
	{
												 ...
												 //		&& ((currentMillis - enc->lastButtonCheck[i]) >= ENC_BUTTONINTERVAL))            // checking enc->button is sufficient every 10-30ms
		{
			...
			{ // key is down
				enc->keyDownTicks[i]++;
				if ((enc->keyDownTicks[i] > (IBTN_HOLDTIME / IENC_BUTTONINTERVAL)) && (enc->buttonHeldEnabled))
				{
					enc->button[i] = Held;
				}
			}
...
#define ENC_SINGLECLICKONLY 1
						...
						{ // prevent trigger in single click mode
							if (enc->doubleClickTicks[i] < (IBTN_DOUBLECLICKTIME / IENC_BUTTONINTERVAL))
							{
							...
							enc->doubleClickTicks[i] = (enc->doubleClickEnabled) ? (IBTN_DOUBLECLICKTIME / IENC_BUTTONINTERVAL) : ENC_SINGLECLICKONLY;
						}
					}
				}
