...
#define BT_BONDED_LIST_ADDR 0x3FF000
...
{ // address, size in BYTES !!!!
	...
	{
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
		ESP_LOGE(TAG, "Warning %lu kmalloc low memory", *eebuf);
#else
...
{ // clear (0) stations and device
	...
	if (buffer != NULL)
	{
		...
		for (i = 0; i < 16; i++)
		{
			...
			vTaskDelay(pdMS_TO_TICKS(10)); // avoid watchdog
		}
...
void eeEraseStations()
{
	...
	if (buffer != NULL)
	{
		...
		ESP_LOGE(TAG, "Warning %s kmalloc low memory", "eeEraseStations");
}
...
void saveStation(struct shoutcast_info *station, uint16_t position)
{
	...
	{
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
		ESP_LOGW(TAG, "Retrying %lu on saveStation", i);
#else
...
void saveMultiStation(struct shoutcast_info *station, uint16_t position, uint8_t number)
{
	...
	{
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
		ESP_LOGW(TAG, "Retrying %lu on SaveMultiStation for %d stations", i, number);
#else
