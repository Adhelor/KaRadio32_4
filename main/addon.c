...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
		ESP_LOGI(TAG, "IR event: Channel: %x, ADDR: %lx, CMD: %lx = %lX, REPEAT: %d",
				 evt.channel, (unsigned long)evt.addr, (unsigned long)evt.cmd, (unsigned long)evtir, evt.repeat_flag);
#else
		ESP_LOGI(TAG, "IR event: Channel: %x, ADDR: %x, CMD: %x = %X, REPEAT: %d",
...
void task_addon(void *pvParams)
{
	...
	if (g_device->lcd_type != LCD_NONE)
	{
		...
		xTaskCreatePinnedToCore (task_lcd, "task_lcd", 2800, NULL, PRIO_LCD, &pxTaskLcd,CPU_LCD); 
	...
	while (1)
	{
		...
		if (_isIR) 	irLoop();  // compute the ir if task started
