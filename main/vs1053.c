...
extern spi_device_handle_t vsspi;  // the evice handle of the vs1053 spi
extern spi_device_handle_t hvsspi; // the device handle of the vs1053 spi high speed
...
bool VS1053_HW_init()
{
	...
	// freq = spi_get_actual_clock(APB_CLK_FREQ, 1400000, 128); // deprecated
	spi_device_get_actual_freq(APB_CLK_FREQ, &freq);
	ESP_LOGI(TAG, "VS1053 LFreq: %lu", freq);
#elif ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(4, 3, 0)
	freq = spi_get_actual_clock(APB_CLK_FREQ, 1400000, 128);
	ESP_LOGI(TAG, "VS1053 LFreq: %d", freq);
#else
	freq = spi_cal_clock(APB_CLK_FREQ, 1400000, 128, NULL);
	ESP_LOGI(TAG, "VS1053 LFreq: %d", freq);
#endif
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
	// freq = spi_get_actual_clock(APB_CLK_FREQ, 6100000, 128); // deprecated
	spi_device_get_actual_freq(APB_CLK_FREQ, &freq);
	ESP_LOGI(TAG, "VS1053 HFreq: %lu", freq);
#elif ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(4, 3, 0)
...
uint16_t VS1053_ReadRegister(uint8_t addressbyte)
{
	...
	ret = spi_device_transmit(vsspi, &t); // Transmit!
	if (ret != ESP_OK)
		ESP_LOGE(TAG, "err: %d, VS1053_ReadRegister(%d), read: %u", ret, addressbyte, *t.rx_data);
...
void VS1053_Start()
{
	...
	if (CheckDREQ() == 0) vTaskDelay(pdMS_TO_TICKS(500)); // wait a bit more
	// Check DREQ
	if (CheckDREQ() == 0)
	{
		vsVersion = 0;
...
{
    // Clamp to 0..60
    if (xMinusHalfdB > 60) xMinusHalfdB = 60;
    // if (xMinusHalfdB < 0) xMinusHalfdB = 0; // uint8_t is always >= 0

    uint8_t att = vs1053_vol_tab[xMinusHalfdB];
    VS1053_WriteRegister(SPI_VOL, att, att);
}
/**
