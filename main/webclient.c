...
const char *icyHeaders[] = {"icy-name:", "icy-notice1:", "icy-notice2:", "icy-url:", "icy-genre:", "icy-br:", "icy-description:", "ice-audio-info:", "icy-metaint:"};
...
void *incmalloc(size_t n)
{
	...
	// ESP_LOGI(TAG, "Incmalloc Free heap size: %lu", esp_get_free_heap_size());
	// ESP_LOGI(TAG, "Largest free block: %u", heap_caps_get_largest_free_block(MALLOC_CAP_DEFAULT));
#else
	// ESP_LOGI(TAG, "Incmalloc Free heap size: %d", esp_get_free_heap_size());
...
{
	test_https();
	// ESP_LOGI(TAG, "bigSram: %d", bigSram());
	if (https && !bigSram())
	{
...
void clientPrintState()
{
	...
	if (!_isRadio)
	{
		// In BT mode, show BT playing state
		extern bool bt_is_playing;
		if (bt_is_playing)
			kprintf("##CLI.PLAYING#\n");
		else
			kprintf("##CLI.STOPPED# from BT\n");
		return;
	}
#endif
	if (playing)
...
static void remove_utf8_bom(char *str)
{
	if (str == NULL)
		return;
	/*  Single BOM removal function
		// Check for BOM at the start
		if ((unsigned char)str[0] == 0xEF &&
			(unsigned char)str[1] == 0xBB &&
			(unsigned char)str[2] == 0xBF)
		{
			// Shift string left by 3 bytes
			memmove(str, str + 3, strlen(str + 3) + 1);
		}
	*/
	// But BOM can be in more places dividing META into segments - parse whole string and get rid of them
	char *src = str;
	char *dst = str;
	while (*src)
	{
...
static void clientSaveMetadata(char *s, int len)
{
	...
	/*If there is a BOM(Byte Order Mark), remove it otherwise The prefix ď»ż will appear*/
	remove_utf8_bom(t);
	// the expurged str
...
bool clientParseHeader(char *s)
{
	...
	if (t != NULL)
	{
		...
		if (strstr(t, "audio/mp3"))
			contentType = KAUDIO_MPEG;
		if (strstr(t, "application/ogg"))
			contentType = KAUDIO_OGG;
		if (strstr(t, "audio/ogg"))
			contentType = KAUDIO_OGG;
...
void clientReceiveCallback(int sockfd, char *pdata, int len)
{
	...
	//	ESP_LOGD(TAG, "Entering clientReceiveCallback: len=%d, pdata=%p, cstatus=%d", len, pdata, cstatus);

	if (pdata == NULL || len <= 0)
	{
		ESP_LOGE(TAG, "Invalid pdata or len in clientReceiveCallback");
		//		return;
	}

	//	ESP_LOGD(TAG, "Processing data: len=%d, pdata=%p, cstatus=%d", len, pdata, cstatus);
	...
	case C_HEADER:
		...
		{
			ESP_LOGD(TAG, "Invalid metaint: %d", header.members.single.metaint);
			//			return;
		}
		metad = -1;
		t1 = strstr(pdata, "302 ");
	...
	case C_HEADER1: // not ended
	{
		...
		do
		{
			...
			if ((t1 != NULL) && (t1 <= pdata + len - 4))
			{
...
#ifndef CONFIG_NO_VS1053
				if (get_audio_output_mode() == VS1053)
					VS1053_SetVolume(1); // lowest volume at start
				else
#endif
					renderer_volume(1); // lowest volume at start
				...
				if (t2 != NULL)
				{
					...
					if (strchr((t1), 0x0A) != NULL)
						*strchr(t1, 0x0A) = 0;
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
					ESP_LOGD(TAG, "chunked: %lu,  strlen: %d  \"%s\"", chunked, strlen(t1) + 1, t1);
#else
				...
				cchunk = chunked;
				//	ESP_LOGD(TAG, "newlen: %d   len: %d   chunked:%" PRIu32 "  pdata:%x", newlen, len, chunked, (int)pdata);
				if (newlen > 0)
			...
			else
			{
				...
				vTaskDelay(pdMS_TO_TICKS(10)); // avoid watchdog is infernal loop
				...
				if (l != 0)
				{
					...
					if (rest < 0)
					{
						*(inpdata + clen) = 0; // truncated
											   //	ESP_LOGD(TAG, "mtlen len:%d, clen:%" PRIu32 ", metad:%d, l:%d, inpdata:%x,  rest:%d", len, clen, metad, l, (int)inpdata, rest);

						if (metadata != NULL)
							incfree(metadata, "meta");
						metadata = incmalloc(l + 1);
						strncpy(metadata, inpdata + metad + 1, l); // truncate if to long
						metadata[l] = '\0';						   // Ensure null termination
					}
					else
						clientSaveMetadata(inpdata + metad + 1, l); // just in case....
				}
				if (metad > 0)
				{
...
void clientTask(void *pvParams)
{
	...
	if ((ret = wolfSSL_CTX_load_verify_buffer(wctx, client_cert_der_2048,
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
// ESP_LOGI(TAG, "IP: %" PRIu32 "  ADDR:%s", dest.sin_addr.s_addr, inet_ntoa(*(struct in_addr *)(serverInfo->h_addr_list[0])));
#else
			...
			if (connect(sockfd, (struct sockaddr *)&dest, sizeof(dest)) >= 0)
			{
				...
				if (https)
				{
					...
					// setSPIRAMSIZE(40*1024); // need heap for ssl connect
					spiRamFifoInit();
					vTaskDelay(pdMS_TO_TICKS(10));
					...
					if (wolfSSL_connect(ssl) != SSL_SUCCESS)
					{
						...
						ESP_LOGE(TAG, "WolfSSL_connect error: %s", wolfSSL_ERR_reason_error_string(err));
						goto NotConnected;
					}
					vTaskDelay(pdMS_TO_TICKS(10)); // wait for wolfSSL connection
				}
				...
				do
				{
					...
					{
						{
							...
							if (bytes_read < 0)
							{
								...
								wolfSSL_ERR_error_string_n(err, buffer, sizeof(buffer) - 1);
					...
					else
					{
						...
						ESP_LOGW(TAG, "#DEBUG Free heap: %d", esp_get_free_heap_size());
						ESP_LOGW(TAG, "#DEBUG Largest free block: %d", heap_caps_get_largest_free_block(MALLOC_CAP_DEFAULT));
						vTaskDelay(pdMS_TO_TICKS(10));
						if ((errno == 128) || (cnterror >= 10))
							break;
					}
					vTaskDelay(pdMS_TO_TICKS(20)); // >1 mandatory
					// if a stop is asked
