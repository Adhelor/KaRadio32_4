...
IRAM_ATTR void serviceJoystick(Joystick_t *enc)
{
	...
	for (uint8_t i = 0; i < NBBUTTONS; i++)
	{
		...
		//	if (((currentMillis - enc->lastButtonCheck[i]) >= ENC_BUTTONINTERVAL))            // checking enc->button is sufficient every 10-30ms
		{
			...
			{ // key is down
				enc->keyDownTicks[i]++;
				if ((enc->keyDownTicks[i] > (JOY_HOLDTIME / JOY_BUTTONINTERVAL)) && (enc->buttonHeldEnabled))
				{
					enc->button[i] = Held;
				}
			}
...
#define ENC_SINGLECLICKONLY 1
						...
						{ // prevent trigger in single click mode
							if (enc->doubleClickTicks[i] < (JOY_DOUBLECLICKTIME / JOY_BUTTONINTERVAL))
							{
						...
						{
							enc->doubleClickTicks[i] = (enc->doubleClickEnabled) ? (JOY_DOUBLECLICKTIME / JOY_BUTTONINTERVAL) : ENC_SINGLECLICKONLY;
						}
					}
				}
				enc->keyDownTicks[i] = 0;
