...
#define CONFIG_BT_SPEAKER_MODE 0
#endif
// lcd_type
// min 128x64
...
#define LCD_SPI_ST7789V3 205 // 170x320

// BT/WebRadio Macros
#define IS_RADIO(x)      (!((x) & (1ULL << 7)))
#define TOGGLE_RADIO(x)  ((x) ^= (1ULL << 7))
#define SET_RADIO(x)     ((x) |= (1ULL << 7))

extern bool _isRadio;
#if defined(CONFIG_DISPLAY_TYPE_MONO)
...
// IR event log for diagnostics
typedef struct {
	uint32_t code;     // Full code (addr << 8) | cmd
	uint16_t addr;
	uint16_t cmd;
	uint8_t repeat;
	uint32_t timestamp;
...
void addonDt();
void wakeLcd();
// Display a short numeric string (PIN) on the device LCD for a few seconds
void showPin(const char *pin, int seconds);
// OTA progress display
void setOtaProgress(int percent);
// Draw progress helpers implemented per-display
void drawProgressU8g2(uint8_t percent);
...
int getBlv();
int getBatPercent();
extern void InitButtonDevices(void);
...
/* Toggle mute: temporarily set volume to 0 and restore previous volume on next call */
void mute(void);
