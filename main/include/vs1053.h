...
static const uint8_t vs1053_vol_tab[24] = {
255, 103, 101, 98, 96,
96, 90, 85, 80, 74, 69, 
64, 58, 53, 48, 43, 37,
32, 27, 21, 16, 11, 5, 0
};
 // from mute to max volume
...
static inline uint8_t VS1053_GetVolume(void)
{
    uint8_t att = VS1053_ReadRegister(SPI_VOL) & 0xFF;
    uint8_t closest = 0;
    uint8_t min_diff = 255;

    for (uint8_t i = 0; i <= 60; i++)
    {
        uint8_t diff = att > vs1053_vol_tab[i] ? att - vs1053_vol_tab[i] : vs1053_vol_tab[i] - att;
        if (diff < min_diff)
        {
            min_diff = diff;
            closest = i;
            if (diff == 0) break; // if exact match found, exit early
        }
    }
