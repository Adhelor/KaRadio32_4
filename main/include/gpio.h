...
// Encoders setting ESP internall pull-up/pull-down
#define ENCPULL_BITS   (0xC0)   // bits 6 and 7
#define ENCPULL_NONE   (0x00)   // 00
#define ENCPULL_PULLUP (0xC0)   // 11
#define ENCPULL_PULLDOWN (0x80) // 10
...
#define PIN_BAT	GPIO_NONE	//GPIO_NUM_32 TO GPIO_NUM_39 or GPIO_NONE if not used.
...
//-------------
#define PIN_SLEEP   GPIO_NONE // 13 . Enter Deep Sleep if pin P_SLEEP is set to P_LEVEL_SLEEP. 
                              // Only GPIOs which have RTC functionality can be used: 0,2,4,12-15,25-27,32-39. 
                            //   And note that GPIO12 is a bootstrap pin, ESP32 might not even start up if GPIO12 is grounded.
...
#define BT_PAIRING_NAMESPACE "bt_pairing"
#define MAX_BT_BONDS CONFIG_BT_SMP_MAX_BONDS
#define BT_BOND_NAME_LEN 32
// to set a value: 		gpio_set_level(gpio_num_t gpio_num,value);
// to read a value: 	int gpio_get_level(gpio_num_t gpio_num)
typedef struct {
    uint8_t mac[6];
    char name[BT_BOND_NAME_LEN + 1];
} bt_bonded_device_t;

typedef struct {
    uint8_t count;
    bt_bonded_device_t devices[MAX_BT_BONDS];
} bt_bonded_list_t;
#endif
