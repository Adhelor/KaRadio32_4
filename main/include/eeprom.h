...
struct device_settings {
	...
	char pass1[PASSLEN];	
	char pass2[PASSLEN];
	char BTname[BTHOSTLEN]; // bluetooth name
	char BTpass[BTPASSLEN]; // bluetooth pass	
	...
	// bit 5&6: log wolfssl  OFF(0)  ERROR&INFO(1) ENTER&LEAVE(2)  OTHER_LOG(3)
...
// esp32
	...
	uint8_t led_brightness; // the backlight brightness in percent
	...
	uint8_t options32;	// bit0:0 = MMDD, 1 = DDMM  in the time display, bit1: 0= lcd without rotation  1 = lcd rotated 180
						...
						// bit5: TOGGLE time or main sreen, bit6: RadioMode(BT Speaker)
	char hostname[HOSTLEN];
	char ota_server_ip[20];
	uint32_t tp_calx;
	uint32_t tp_caly;
