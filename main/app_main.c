...
{
	// ESP_LOGD(TAG, "kmalloc Free heap size: %d", esp_get_free_heap_size());
	// ESP_LOGDTAG, "Largest free block: %d", heap_caps_get_largest_free_block(MALLOC_CAP_DEFAULT));
	if (bigRam)
...
{
	// ESP_LOGD(TAG, "kcalloc Free heap size: %d", esp_get_free_heap_size());
	// ESP_LOGD(TAG, "Largest free block: %d", heap_caps_get_largest_free_block(MALLOC_CAP_DEFAULT));
	if (bigRam)
...
void initTimers()
{
	...
	#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
	ESP_ERROR_CHECK(gptimer_enable(mstimer)); //for IDF > 5 need to enable timer first
	#endif
...
static renderer_config_t *create_renderer_config()
{
	...
	if (renderer_config->output_mode == I2S_MERUS || I2S_32BIT)
	{
...
static void start_wifi()
{
	...
	while (1)
	{
		...
		if ((xEventGroupWaitBits(wifi_event_group, CONNECTED_AP, false, true, 4000) & CONNECTED_AP) == 0)
...
void start_network()
{
	...
	else // mode STA
	{
		...
		// wait for ip
		if ((xEventGroupWaitBits(wifi_event_group, CONNECTED_BIT, false, true, 4000) & CONNECTED_BIT) == 0) // timeout
...
void app_main()
{
...
#else
	ESP_LOGD(TAG, "Running partition type %d subtype %d (offset 0x%x)",
			 running->type, running->subtype, running->address);
#endif
	// Initialize NVS.
	err = nvs_flash_init();
	...
	xTaskCreatePinnedToCore(timerTask, "timerTask", 2560, NULL, PRIO_TIMER, &pxCreatedTask, CPU_TIMER);
	ESP_LOGI(TAG, "%s task: %x", "t0", (unsigned int)pxCreatedTask);

	xTaskCreatePinnedToCore(uartIfaceTask, "uartIfaceTask", 2560, NULL, PRIO_UART, &pxCreatedTask, CPU_UART);
	...
	//-----------------------------
	/* init wifi & network*/
	// ESP_LOGI(TAG, "RAM left bw: %u", esp_get_free_heap_size());
	start_wifi();
	// ESP_LOGI(TAG, "RAM left aw: %u", esp_get_free_heap_size());
	start_network();
	// ESP_LOGI(TAG, "RAM left an: %u", esp_get_free_heap_size());
	...
	vTaskDelay(1);
	xTaskCreatePinnedToCore(clientTask, "clientTask", 9216, NULL, PRIO_CLIENT, &pxCreatedTask, CPU_CLIENT);
	...
	xTaskCreatePinnedToCore(serversTask, "serversTask", 2560, NULL, PRIO_SERVER, &pxCreatedTask, CPU_SERVER);
	...
	xTaskCreatePinnedToCore(task_addon, "task_addon", 3072, NULL, PRIO_ADDON, &pxCreatedTask, CPU_ADDON);
