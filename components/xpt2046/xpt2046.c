...
void saveCalibration(uint32_t x, uint32_t y)
{
	...
	ESP_LOGI(TAG, "Calibration saved: X:%lx, Y:%lx", x, y);
#else
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
	ESP_LOGI(TAG, "Calibration read: X:%lx, Y:%lx", tp_calx, tp_caly);
#else
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
	ESP_LOGV(TAG, "xleft:%d,xright:%d,ytop:%d,ybottom:%d,tp_calx:%lu,tp_caly:%lu,X:%lu,Y:%lu", xleft, xright, ytop, ybottom, tp_calx, tp_caly, X, Y);
#else
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
	ESP_LOGD(TAG, "Rotate: %d, X:%li,Y:%li", getRotat(), X, Y);
#else
